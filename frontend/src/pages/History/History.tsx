...
export default function History() {
  const { convertTemp, tempUnit, refreshInterval } = useSensorData();

  const [data, setData] = useState<any[]>([]);
  const [viewMode, setViewMode] = useState<string>(
    () => localStorage.getItem("history_viewMode") || "combined",
  );
  ...
  const [startDate, setStartDate] = useState<string>(
    () => localStorage.getItem("history_startDate") || "",
  );
  const [endDate, setEndDate] = useState<string>(
    () => localStorage.getItem("history_endDate") || "",
  );
  ...
  useEffect(() => {
    localStorage.setItem("history_dataCount", dataCount.toString());
  }, [dataCount]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async (isBackgroundFetch = false) => {
      if (dataCount === 0 && (!startDate || !endDate)) {
        if (isMounted) setData([]);
        if (isMounted && !isBackgroundFetch) setLoading(false);
        return;
      }

      if (!isBackgroundFetch && isMounted) {
        setLoading(true);
      }
      ...
      try {
        ...
        if (dataCount === 0) {
          ...
          fetchUrl = `${Endpoints.History}?from=${fromIso}&to=${toIso}&count=1000000`;
        }
        ...
        if (response.ok && isMounted) {
          ...
          const groupedData = result.reduce((acc: any, curr: SensorReading) => {
            ...
            if (curr.type === 0 && curr.temperatureC != null) {
              acc[groupKey].temperatureC = curr.temperatureC;
            }
            if (curr.type === 1 && curr.humidityPercent != null) {
              acc[groupKey].humidityPercent = curr.humidityPercent;
            }
        ...
        if (isMounted && !isBackgroundFetch) {
          setTimeout(() => {
  ...
  const handleDownloadCsv = () => {
    if (dataCount === 0 && startDate && endDate) {
      ...
      const toIso = new Date(`${endDate}T23:59:59`).toISOString();
      window.open(
        `${Endpoints.ReportCsv}?from=${fromIso}&to=${toIso}`,
        "_blank",
      );
    } else if (dateRange) {
      window.open(
        `${Endpoints.ReportCsv}?from=${dateRange.from}&to=${dateRange.to}`,
        "_blank",
      );
    } else {
      ...
      <div className="chart-card">
        {loading ? (
          ...
          <div className="empty-chart-state">
            ...
            <p>
              Für den gewählten Zeitraum sind aktuell keine historischen
              Sensordaten vorhanden. Sobald der Sensor neue Werte aufzeichnet,
              erscheint hier das Diagramm.
            </p>
          </div>
        ) : (
